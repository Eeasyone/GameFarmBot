...
pytesseract.pytesseract.tesseract_cmd = r'D:\Tesseract\tesseract'                       # Путь к файлу запуска ИИ
                ...
                #ПРОВЕРКА ЛОКАЛА
def main():
    left, top, one, two = pyautogui.locateOnScreen("Screenshot_2.jpg", confidence=.85)              # Ищем на экране нейтрала для формулы
    image = ImageGrab.grab(bbox=(left, top-28, left+103, top+47))                                 
    image1 = ImageGrab.grab(bbox=(left-95, top-28, left-45, top+47))
    image_parse = pytesseract.image_to_string(image, config='outputbase digits')
    image_parse1 = pytesseract.image_to_string(image1, config = 'outputbase digits', lang = 'eng')
    
    while True:                                                                                            
        if int(image_parse) != 0 and int(image_parse1) != 0:
                image = ImageGrab.grab(bbox=(left, top-28, left+103, top+47))
                image1 = ImageGrab.grab(bbox=(left-95, top-28, left-45, top+47))
                image_parse = pytesseract.image_to_string(image, config='outputbase digits')
                image_parse1 = pytesseract.image_to_string(image1, config = 'outputbase digits', lang = 'eng')
                print(image_parse, image_parse1)
